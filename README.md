...
- TTS giọng Việt: Gemini-TTS (cloud, mặc định Aoede), VieNeu-TTS chạy local (miễn phí,
  hỗ trợ nhân bản giọng), hoặc Vbee (cloud VN, giọng tự nhiên); vòng viết-lại để TTS
  đọc vừa khung giờ.
...
Vertex AI như cũ. Khi engine là `vbee`, chọn giọng qua `VIDEO_DUB_VBEE_VOICE` (lựa chọn
Aoede/Kore… trên UI không có tác dụng).
